...
function isObject(value) {
  ...
  if (value.constructor === RegExp) {
    return false;
  } else if (value.constructor === window.Map) {
    return false;
  } else if (value.constructor === window.Set) {
    return false;
  } else if (value.constructor === window.Date) {
    return false;
  }
...
function throwOnBadArgs(givenStyle, windowHeight) {
  if (!isObject(givenStyle) && givenStyle !== undefined) throw Error("style (the first argument) must be an object or undefined");
